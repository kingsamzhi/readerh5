...
public class ExcelUtils {
     ...
     */
    public static <T>  List<T>  importExcel(File multipartFile, Class<T> clz) throws IOException, NoSuchMethodException, IllegalAccessException, InvocationTargetException, InstantiationException, InvalidFormatException {
        ...
        log.info("文件类型:{}", FileUtils.getFileTypeByFile(multipartFile));
        String fileName = multipartFile.getName();
        log.info("文件名:{}", fileName);

        if(!"zip".equals(FileUtils.getFileTypeByFile(multipartFile))) {
        ...
        for (int j = 0; j < fields.length; j++){
            ...
            if (field.isAnnotationPresent(ExcelHeader.class)) {
                ...
                String e1=getCellValue(cell).toString();
                String e2=annotation.value().toString();
                if (cell == null || !getCellValue(cell).toString().equals(annotation.value().toString())) {
